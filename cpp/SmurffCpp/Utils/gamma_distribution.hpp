...
public:
    ...
    result_type operator()(Engine& eng)
    {
        ...
        if(_alpha == result_type(1)) {
            ...
            for(;;) {
                result_type y = tan(pi * std::uniform_real_distribution<RealType>(0., 1.)(eng));
                result_type x = sqrt(result_type(2)*_alpha-result_type(1))*y
                    + _alpha-result_type(1);
                if(x <= result_type(0))
                    continue;
                if(std::uniform_real_distribution<RealType>(0., 1.)(eng) >
            ...
            for(;;) {
                result_type u = std::uniform_real_distribution<RealType>(0.,1.)(eng);
...
private:
    ...
    std::exponential_distribution<RealType> _exp;
    result_type _alpha;
    result_type _beta;
